...
export default function InternalLinks({ title = "Related Topics", links, currentPath }) {
    ...
    return (
        <section className="mt-12 bg-gray-50 rounded-2xl p-6 md:p-8 border border-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredLinks.map((link, idx) => (
                    <Link
                        key={idx}
                        to={link.path}
                        className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all duration-200 group"
                    >
                        <div className="flex items-center gap-3 min-w-0">
                            {link.icon && <link.icon className="w-5 h-5 text-primary shrink-0" />}
                            <div className="min-w-0">
                                <p className="font-semibold text-gray-900 group-hover:text-primary transition-colors text-sm truncate">
                                    {link.title}
                                </p>
                                {link.subtitle && (
