                ...
                <div className="max-w-6xl mx-auto">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-3 border-2 border-gray-100 rounded-xl focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none"
                                />
                            </div>
                            <div className="flex gap-2 flex-wrap">
                                {categories.map(cat => (
                                    <button
                                        ...
                                        className={`px-4 py-2.5 rounded-lg font-medium text-sm transition-all ${activeCategory === cat.id ? 'bg-primary text-white shadow-soft' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}
                                    >
                                        {cat.label}
                                    </button>
                                ))}
                    ...
                    {filtered.length === 0 ? (
                        <div className="text-center py-16">
                            ...
                            <p className="text-gray-500 font-medium">No templates found for your search.</p>
                        </div>
                    ) : (
                        <>
                            <p className="text-sm text-gray-500 mb-6">Showing {filtered.length} template{filtered.length !== 1 ? 's' : ''}</p>
                    ...
                    <div className="mt-12 p-6 bg-primary/5 rounded-2xl border border-primary/10">
                        <h2 className="text-xl font-bold text-gray-900 mb-2">How to Use These Templates</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                            {[
                                ...
                                <div key={s.step} className="flex gap-3 p-4 bg-white rounded-xl border border-gray-100">
                                    ...
                                    <div><p className="font-bold text-sm">{s.title}</p><p className="text-xs text-gray-500 mt-0.5">{s.desc}</p></div>
                                </div>
                            ))}
                        </div>
                    </div>
